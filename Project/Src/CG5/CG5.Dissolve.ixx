export module Lumina.CG5.Dissolve;
...
namespace Lumina {
	...
	public:
		...
		void SetPipeline(
			...
			D3D12_GPU_DESCRIPTOR_HANDLE srv_MaskTexture_
		) const {
			...
			cmdList_->SetGraphicsRootDescriptorTable(2U, srv_MaskTexture_);
		...
		void Initialize(
			...
			auto settings{ Utils::LoadFromFile<nlohmann::json>("Dissolve.json", "Assets/CG5") };
			auto rsSetup{ DX12::LoadRootSignatureSetup(settings.at("RS")) };
			RS_.Initialize(device_, rsSetup, "Dissolve RS");
